...
Experimentalmente medimos um offset na ordem de $-700 \text{mV}$, o que é bastante elevado. Como apenas iriamos amplificá-lo na fase de amplificação, decidimos fazer primeiro a correção do offset:
...
V_{1}=G(OUT_{+}-OUT_{-}) \\
V_{1}-R_{amp}I=0 \\
V_{2}+(P_{1}+R_{P1}+R_{P2})I=0 \\
\end{cases}$$
de onde podemos obter:
$$V_{2}=-\frac{P_{1}+R_{P1}+R_{P2}}{R_{amp}}V_{1}$$
logo temos:
$$V_{2}=-6.8 \frac{P_{1}+R_{P1}+R_{P2}}{R_{amp}}\left(5+\frac{80\cdot10^{3}}{R_{G}}\right)\frac{R_{3}+R_{4}}{R_{2}+R_{3}}\frac{R_{x}}{R_{0}}$$
Invés de colocar um potenciómetro de $10\text{k}\Omega$, colocamos um potenciometro $P_{1}$ de $1\text{k}\Omega$ e duas resistências $R_{P1},R_{P2}$. Fizemos isto de modo a poder controlar o offset com maior precisão, tendo-se o ganho adequado para o funcionamento da balança. Na secção de Calibração veremos como foram decididas as resistências.
...
![[balanca filt.png]]
Agora vemos um sinal que se aparenta bastante mais a uma sinusoide, com uma aparência mais "limpa".
Além disso, quando se coloca uma massa na balança podemos ver o seguinte:
![[balança colocar massa.png]]
Foi ainda possível medir uma frequência de $50\text{Hz}$. Isto corresponde à frequência da rede elétrica, que não é eliminada na filtragem do sinal, porque esta é medida devido a efeito de antena nos fios (??????????). Além disso, vimos que o circuito demora cerca de $0.3\text{s}$ a reagir à colocação de uma massa na balança, o que consideramos apropriado.

Para melhor compreender o efeito da filtragem sobre o sinal fizemos FFT dos dois sinais, tendo-se:
![[balanca fft python.png]]
No gráfico do sinal sem filtragem vemos que temos um pico principal em $50 \text{Hz}$, que é a frequência da rede. Além dele, temos picos em $150 \text{Hz}, 250 \text{Hz}, 350 \text{Hz},\dots$ (em intervalos de 100 Hz). Vemos que estes picos menores têm aproximadamente um décimo da amplitude do principal, tendo então um peso relevante na onda observada.

No gráfico do sinal com filtragem continuamos a ter um pico em $50 \text{Hz}$, a frequência da rede. Novamente temos os picos em intervalos de 100Hz. Concluimos que o sinal da rede não deverá ser uma sinusoide perfeita, mas sim composto por estas frequências secundárias, de tal modo que nunca são completamente excluídas. No entanto, agora os picos menores apenas têm cerca de um centésimo da amplitude do pico principal. Assim, não têm tanto peso e temos um sinal próximo de uma sinusoide perfeita. O pico principal agora decai de forma suave e não abrupta, o que se deverá à resposta do filtro a estas frequências (?????).

Conforme as FFTs observadas, podemos concluir que o filtro MFB de 4ª ordem terá funcionado de forma correta, tendo anulado todas as frequências, exceto as da rede. Além disso, o pico principal tornou-se significativamente mais intenso, tornado-se o sinal mais "puro". Infelizmente, devido à reduzida resolução em frequência da imagem obtida, não conseguimos verificar se o filtro de facto teve uma frequência de corte de cerca de $10 \text{Hz}$.

## Calibração
![[bal_total.png]]
Conforme o que foi visto acima, temos que a tensão medida no final do circuito, $V_{3}$ será:
$$V_{3}=1.68\cdot6.8 \frac{P_{1}+R_{P1}+R_{P2}}{R_{amp}}\left(5+\frac{80\cdot10^{3}}{R_{G}}\right)\frac{R_{3}+R_{4}}{R_{2}+R_{3}}\frac{R_{x}}{R_{0}}$$
...
$$V_{3}=14.5\frac{P_{1}+R_{P1}+R_{P2}}{R_{amp}}R_{x}$$
ora, para o declive ser 50 é necessária a relação:
$$P_{1}+R_{P1}+R_{P2}=3.45 R_{amp}$$
Desta forma, colocamos $R_{amp}=2.2\text{k}\Omega$, de modo que $3.45R_{amp}=7590\Omega$. Assim, colocamos $R_{P1}=6800\Omega,R_{P2}=470\Omega$, de modo que ao ajustar o potenciometro a resistência total do ramo varia no intervalo $[6270, 8270]\Omega$.
...
Ora, acima consideramos a relação teórica: $R_{x}=0.2w$ ($R_{x}$ em $\Omega$, $w$ em kg). Com esta vimos que ocorre calibração quando $P_{1}+R_{P1}+R_{P2}=3.45R_{amp}$. No entanto, decidimos medir $P_{1},R_{amp}$ depois da calibração. Verificamos que, na realidade temos a relação: $$P_{1}+R_{P1}+R_{P2}=3.351 R_{amp}$$
